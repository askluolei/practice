<template>
  <!-- copy from https://github.com/1615450788/vue-cron/blob/master/src/index.vue -->
  <div id="changeContab">
          ...
          <el-row>
            <el-radio v-model="day.cronEvery" label="2">每隔
              ...
              <el-select size="small" v-model="week.incrementStart">
                <el-option v-for="val in 7" :key="val" :label="'星期'+['天','一','二','三','四','五','六'][val-1]" :value="val"></el-option>
              </el-select>
              天开始
            </el-radio>
          </el-row>
          ...
          <el-row>
            <el-radio class="long" v-model="day.cronEvery" label="4">具体星期几(可多选)
              <el-select size="small" multiple v-model="week.specificSpecific">
                <el-option v-for="val in 7" :key="val" :label="'星期'+['天','一','二','三','四','五','六'][val-1]" :value="['SUN','MON','TUE','WED','THU','FRI','SAT'][val-1]"></el-option>
              </el-select>
            </el-radio>
          </el-row>
          <el-row>
          ...
          <el-row>
            <el-radio class="long" v-model="month.cronEvery" label="3">具体月份(可多选)
              <el-select size="small" multiple v-model="month.specificSpecific">
                <el-option v-for="val in 12" :key="val" :label="val+'月'" :value="val"></el-option>
              </el-select>
            </el-radio>
          </el-row>
          <el-row>
            <el-radio v-model="month.cronEvery" label="4">从
              <el-input-number size="small" v-model="month.rangeStart" :min="1" :max="12"></el-input-number>月 到
          ...
          <el-row>
            <el-radio class="long" v-model="year.cronEvery" label="3">具体年份(可多选)
              <el-select size="small" filterable multiple v-model="year.specificSpecific">
                <el-option v-for="val in 100" :key="val" :label="2016+val+'年'" :value="2016+val"></el-option>
              </el-select>
            </el-radio>
          </el-row>
          <el-row>
            <el-radio v-model="year.cronEvery" label="4">从
              <el-input-number size="small" v-model="year.rangeStart" :min="2017" :max="2117"></el-input-number>年 到
  ...
  export default {
    ...
    computed: {
      ...
      cron() {
        ...
        let secondsText = this.secondsText || '*'
        let minutesText = this.minutesText || '*'
        let hoursText = this.hoursText || '*'
        let daysText = this.daysText || '*'
        let monthsText = this.monthsText || '*'
        let weeksText = this.weeksText || '?'
        const yearsText = this.yearsText || '*'
        switch (index) {
          case 'second':
            secondsText = 0
            break
    ...
    mounted() {
      if (this.$props.data) {
        const array = this.$props.data.split(' ')
        console.log(array)
      } else {
