...
<script>
  ...
  methods: {
    ...
    codeChangeHandler: function(editor, event) {
      const lines = []
      if (editor.getModel()._lines) {
        for (const line of editor.getModel()._lines) {
          lines.push(line.text)
        }
      }
      const code = lines.join('\n')
      if (this.codeChangeEmitter) {
        this.codeChangeEmitter(code);
      } else {
        this.codeChangeEmitter = _.debounce(
          function(code) {
